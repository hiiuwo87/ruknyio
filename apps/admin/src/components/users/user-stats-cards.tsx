...
/**
 * 📊 User Stats Cards
 * Dashboard-style stats overview for users
 */
...
export function UserStatsCards({ stats }: { stats: UserStatsData }) {
  const cards: Array<{
    title: string;
    value: string;
    change: string;
    ...
    },
    {
      ...
      change: `${stats.verificationRate}% rate`,
      trend: stats.verificationRate > 50 ? "up" : "down",
      icon: UserCheck,
    },
    {
      title: "2FA Enabled",
      value: formatNumber(stats.twoFactorEnabled),
      change: `${stats.total > 0 ? Math.round((stats.twoFactorEnabled / stats.total) * 100) : 0}% of total`,
      trend: "up",
      icon: ShieldCheck,
    },
    {
      title: "Premium",
      value: formatNumber(stats.byRole.premium + stats.byRole.admin),
...
export function UserStatsCardsSkeleton() {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
